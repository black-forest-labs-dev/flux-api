"""FLUX (Black Forest Labs) API client — thin, dependency-light wrapper over the Synexa prediction API.
...
MODELS = {
    ...
    "black-forest-labs/flux-1.1-pro": {
        ...
        "url": "https://synexa.ai/explore/black-forest-labs/flux-1.1-pro"
    },
    ...
    "black-forest-labs/flux-2-klein-9b": {
        ...
        "url": "https://synexa.ai/explore/black-forest-labs/flux-2-klein-9b"
    }
}
...
class Client:
    def __init__(self, api_key: Optional[str] = None, base_url: str = API_BASE, timeout: float = 60.0):
        ...
        self._http = httpx.Client(timeout=timeout, headers={"x-api-key": self.api_key, "user-agent": "flux_api/0.1.0"})
...
def run(input: Dict[str, Any], model: str = DEFAULT_MODEL, **kw):
    """Module-level shortcut: `flux_api.run({...})` using SYNEXA_API_KEY from the environment."""
    global _default
    if _default is None:
        _default = Client()
